...
[минимишки.рф](https://минимишки.рф/) служит справочным источником для разделов и услуг,
но новая версия получает новый дизайн и состав, согласованный для нового сайта. Изменяемая
...
- Ассистент самостоятельно редактирует файлы проекта при выполнении явно запрошенной
  задачи. Изменения должны оставаться в её согласованной области; коммит без отдельной
  просьбы пользователя не создаётся.
- После каждого шага обновляется журнал прогресса.
- Фраза пользователя «Сделай фичу полностью сам» означает, что ассистент полностью
  реализует текущую согласованную задачу без промежуточных остановок и прерываний,
  включая проверки и код-ревью. Уточнение запрашивается только когда без него нельзя
  безопасно определить результат, чтобы не делать предположений за пользователя.
- Если пользователь говорит «завершаем сессию», перед финальным ответом ассистент
  сохраняет в `.Codex/plans/` всё, что нужно для продолжения без потери контекста:
  фактическое состояние шагов, незавершённую точку, принятые решения, результаты
  проверок, Git-состояние и точное следующее действие.
...
## ✂️ Формат небольших правок

Если в существующем файле меняется только небольшая, локально ограниченная часть,
ассистент не обязан выводить файл целиком. В таком случае он указывает точное место
вставки нового кода или конкретный фрагмент, который нужно заменить. Полный файл
выдаётся для нового файла, крупной переработки или когда частичная замена создаёт
риск ошибки при ручном применении.
