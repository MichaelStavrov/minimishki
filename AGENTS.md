...
- По явно выраженной просьбе пользователя ассистент продолжает согласованную задачу без
  промежуточных остановок и завершает её вместе с проверками и код-ревью. Уточнение
  запрашивается только когда без него нельзя безопасно определить результат.
