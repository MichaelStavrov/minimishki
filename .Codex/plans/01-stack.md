...
| Качество кода | **ESLint 9.x** (flat config, type-aware правила) + Prettier — общие конфиги в `packages/eslint-config` |
...
| **TypeScript — только `^6.x`** | `latest` брать **нельзя**: в реестре это 7.x, несовместимый с тулингом (см. врезку выше). Проверять командой `pnpm view typescript@6 version` |
| **ESLint — только `^9.x`** | Второй пакет, где `latest` брать **нельзя**. На ESLint 10 `eslint-config-next` падает при запуске: плагины `react`, `import`, `jsx-a11y` объявляют peer до `^9`, а 10-я версия убрала legacy-API правил. Проверять командой `pnpm view eslint dist-tags.maintenance`. Вместе с ним в 9-й ветке держится `@eslint/js` — он версионируется синхронно с ядром |
