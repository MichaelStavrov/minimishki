...
Решение подтверждено 25.08.2026.

`Post` представляет как обычную новость, так и анонс разового события. Полный
текст хранится в `contentHtml` и проходит ту же серверную очистку по allowlist,
что и HTML услуг. Обложка и галерея остаются отдельными структурированными полями.

Необязательные параметры события:

```text
eventStartsAt: DateTime | null
eventEndsAt: DateTime | null
...
Текстовые `ageLabel` и `priceLabel` выбраны намеренно: условия разовых событий
могут иметь произвольную формулировку, а фильтрация и вычисления по ним в первой
версии не требуются. Если указаны обе даты события, `eventEndsAt` должна быть
позже `eventStartsAt`. Все входные `DateTime` обязаны содержать время и явный
часовой пояс (`Z` либо смещение вида `+03:00`), чтобы результат не зависел от
часового пояса процесса Node.js.

`registrationUrl` принимает только внутренний путь с одним начальным `/` либо
полный HTTP(S)-адрес с непустым хостом. Пустая HTML-разметка визуального редактора
вроде `<p><br></p>` не считается содержимым и отклоняется после очистки.

Публикация использует комбинированную семантику:

- при переходе в опубликованное состояние без `publishedAt` backend подставляет
  текущее время;
- администратор может задать или изменить `publishedAt`, в том числе для переноса
  исторической публикации или планирования;
- запись с будущим `publishedAt` не видна в публичном API до наступления этого времени;
- снятие с публикации и повторная публикация не меняют существующую дату автоматически;
- публична только запись с `isPublished = true`, непустым `publishedAt` и
  `publishedAt <= now()`.

Инварианты публикации и периода события дополнительно защищаются PostgreSQL:
опубликованная запись обязана иметь `publishedAt`, а `eventEndsAt` допускается
только вместе с `eventStartsAt` и должна быть позже начала.

Обновление блокирует строку `Post` через `SELECT ... FOR UPDATE` внутри Prisma-
транзакции. Два одновременных PATCH поэтому последовательно читают актуальное
состояние и не могут собрать несовместимую пару `isPublished` / `publishedAt`.

`Post` удаляется физически. Связанные строки `GalleryItem` удаляются PostgreSQL
по `onDelete: Cascade`; удаление самих файлов из хранилища остаётся отдельной операцией.

### Галерея

Решение подтверждено 25.08.2026.

- backend-модуль и HTTP-маршрут называются `gallery-items`;
- текущий CRUD управляет только записями и принимает готовый URL; загрузка файлов
  остаётся отдельной задачей после выбора хранилища;
- публичный `GET /api/gallery-items` возвращает только опубликованные фотографии общей
  галереи, у которых одновременно `postId = null` и `serviceId = null`; фотографии
  услуг и публикаций выдаются внутри соответствующих карточек и здесь не дублируются;
- административный список и CRUD доступны ролям `ADMIN` и `MANAGER`;
- фотографию разрешено переносить между общей галереей, услугой и публикацией через
  `PATCH`, но одновременно может быть задан не более чем один владелец;
- при назначении владельца API проверяет существование услуги или публикации до записи;
- удаление `GalleryItem` физическое и удаляет только строку метаданных, но не файл
  по сохранённому URL.
