...
**Следующий шаг: № 19 — seed-данные** (этап E).
Шаг 18 закрыт: глобальный `JwtAuthGuard` закрыл приложение по умолчанию,
`RolesGuard` ограничивает маршруты по ролям, а `@Public()` явно открывает login и health.
`GET /api/auth/me` возвращает актуального пользователя из базы по `sub` проверенного JWT.
Код-ревью полного дифа шагов 17–18 проведено, найденные дефекты исправлены и проверены.

При старте новой сессии:
1. Прочитать [`00-process.md`](./00-process.md) — требования к процессу работы.
2. Если шаг 18 ещё не закоммичен — предложить сообщение коммита и дождаться завершения PR.
3. Затем выдать код шага 19 — `prisma/seed.ts`.
   Вместе с ним закрыть зафиксированные долги: `prisma/tsconfig.json`, переменные
   `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD` и предупреждение Prisma о конфигурации seed.
...
### Этап E — backend, аутентификация ← **текущий**
