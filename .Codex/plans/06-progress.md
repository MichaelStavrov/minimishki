...
**Следующий шаг: № 11 — конфиги `apps/api`** (этап D, backend и база).
Этап C закрыт: Postgres поднят в Docker, переменные окружения заведены.

При старте новой сессии:
1. Прочитать [`00-process.md`](./00-process.md) — требования к процессу работы.
2. Выдать код шага 11 — описание в [`03-steps.md`](./03-steps.md). Файлов четыре:
   `package.json`, `tsconfig.json`, `nest-cli.json`, `eslint.config.mjs`.
   Backend пользователь не знает: пояснения развёрнутые.
   ⚠️ Две записанные находки, которые всплывут именно здесь:
   **`rootDir: "./src"` задавать нельзя** (конфликт с `prisma/seed.ts`, `TS6059`)
   и **открытый вопрос по `turbo.json`** — задача `dev` без `dependsOn: ["^build"]`
   стартует API раньше сборки `shared`. Второе меняет принятое решение,
   поэтому выносится пользователю (см. `00-process.md`, п. 4).
