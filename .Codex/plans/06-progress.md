...
**Следующий шаг: № 17 — аутентификация, часть 1** (этап E).
Шаг 16 закрыт: модуль пользователей работает, CRUD проверен запросами.
Долги ревью сессии 8 № 4 и № 5 закрыты — `enum-parity.ts` и `serialize.ts` на месте,
`as unknown as` в проекте нет.

При старте новой сессии:
1. Прочитать [`00-process.md`](./00-process.md) — требования к процессу работы.
2. Выдать код шага 17 — описание в [`03-steps.md`](./03-steps.md): `src/auth/` —
   `auth.service.ts`, `auth.controller.ts`, `dto/login.dto.ts`.
   ⚠️ Отступление от плана шага: **хеширование уже сделано на шаге 16** в `UsersService`
   (решение пользователя, см. таблицу). `AuthService` только сверяет пароль через
   `argon2.verify` и выдаёт JWT. Готовое: `UsersModule` экспортирует `UsersService`,
   метод `findByEmailWithHash(email)` возвращает тип `UserWithHash` (поля `UserDto`
   плюс `passwordHash`), email внутри нормализуется сам.
