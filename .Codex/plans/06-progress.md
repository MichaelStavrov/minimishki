...
**Следующий шаг: № 18 — аутентификация, часть 2** (этап E).
Шаг 17 закрыт: `POST /api/auth/login` проверяет пароль через Argon2 и выдаёт JWT
с `sub`, `email`, `role` на 30 дней. Успешный и ошибочные сценарии проверены живьём.
`GET /api/auth/me` перенесён в шаг 18: маршрут зависит от JWT-стратегии,
глобального guard и `@CurrentUser()`, которых до этого шага ещё нет.

При старте новой сессии:
1. Прочитать [`00-process.md`](./00-process.md) — требования к процессу работы.
2. Выдать код шага 18 — описание в [`03-steps.md`](./03-steps.md): JWT-стратегия,
   guards, `@Public()`, `@Roles()`, `@CurrentUser()` и `GET /api/auth/me`.
   Обязательно закрыть долги: `@Public()` на health и login, `@Roles(ROLE.ADMIN)`
   на маршрутах users. После подключения глобального guard приложение должно быть
