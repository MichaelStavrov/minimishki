...
**Следующий шаг: № 16 — модуль пользователей** (этап E, аутентификация).
Шаги 13–15 закрыты, 🔧 **КТ-3 пройдена**: API поднимается, `GET /api/health` отдаёт `200`.
Этап D закрыт целиком.

При старте новой сессии:
1. Прочитать [`00-process.md`](./00-process.md) — требования к процессу работы.
2. Выдать код шага 16 — описание в [`03-steps.md`](./03-steps.md): `src/users/` —
   `users.module.ts`, `users.service.ts`, `users.controller.ts`, `dto/`.
   На этом шаге `apps/api` **впервые импортирует** `@minimishki/shared`, поэтому здесь же
   уместно закрыть два долга из ревью сессии 8: создать `src/common/enum-parity.ts`
   и решить, как объекты Prisma (`Date`) превращаются в DTO (`string`).
...
- [ ] Шаг 16 — `src/users/` (+ долги из ревью: `src/common/enum-parity.ts`, маппинг дат)
