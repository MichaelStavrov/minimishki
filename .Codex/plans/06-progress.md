...
**Шаг № 19 — seed-данные завершён** (этап E) в ветке `feat/seed-data`.
...
и seed. Изменения ещё не закоммичены.

**Следующий шаг после ревью, коммита и PR: № 20.1 — `modules/courses/`** (этап F).

При старте новой сессии:
1. Прочитать [`00-process.md`](./00-process.md) — требования к процессу работы.
2. Убедиться, что активна ветка `feat/seed-data`, а изменения шага 19 остаются
   незакоммиченными.
3. Предложить сообщение коммита для завершённого и проверенного шага 19.
4. Провести ветку через PR в `main`.
5. Шаг 20.1 начинать в отдельной ветке от обновлённого `main`.
...
### Этап F — backend, доменные модули ← **следующий**
